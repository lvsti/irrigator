...
HumidityLoggerClass HumidityLogger;

HumidityLoggerClass::HumidityLoggerClass(): 
    _lastSampleCumulativeTime(CumulativeTime::distantPast()) {
    pinMode(pinA[0], INPUT);
}

int HumidityLoggerClass::sample() {
    ...
    if (seconds > 0) {
        LOG(F("[HumidityLogger] not sampling.\n"));
        return -1;
    }
    ...
    LOG(String(F("[HumidityLogger] humidity = ")) + String(value) + 
...
bool HumidityLoggerClass::submitToIOTPlotter(int value) {
    ...
    if (!WiFi.hostByName("iotplotter.com", logServerIP)) {
        LOG(F("[HumidityLogger] error: cannot resolve log server host\n"));
        return false;
    }
    ...
    if (!client.connected()) {
        LOG(F("[HumidityLogger] error: cannot connect to log server\n"));
        return false;
    }

    String payload("{\"data\":{\"humidity\":[{\"value\":" + String(value) + "}]}}");
    ...
    if (!client.available()) {
        LOG(F("[HumidityLogger] error: connection reset\n"));
        return false;
    }

    HTTPResponse response(client);
    if (response.statusCode() != 200) {
        LOG(String(F("[HumidityLogger] error: server returned ")) + String(response.statusCode()) + F("\n"));
        return false;
    }
